...
describe('detectProjectType', () => {
  ...
  test('returns "backend-ts" for package.json with no dependencies', async () => {
    fs.writeFileSync(path.join(tmpDir, 'package.json'), JSON.stringify({ name: 'my-lib' }));
    expect(await detectProjectType(tmpDir)).toBe('backend-ts');
  });
...
describe('parseWorktreePorcelain', () => {
  ...
  test('strips refs/heads/ prefix from branch', () => {
    const output = ['worktree /repo/work', 'HEAD abc123', 'branch refs/heads/my-branch', ''].join(
      '\n',
    );

    expect(parseWorktreePorcelain(output)).toEqual([{ path: '/repo/work', branch: 'my-branch' }]);
  });
  ...
  test('skips bare repo entry (no branch line)', () => {
    ...
    expect(parseWorktreePorcelain(output)).toEqual([{ path: '/repo/feature', branch: 'feature' }]);
  });

  test('handles last entry without trailing newline', () => {
    const output = ['worktree /repo/work', 'HEAD abc123', 'branch refs/heads/main'].join('\n');

    expect(parseWorktreePorcelain(output)).toEqual([{ path: '/repo/work', branch: 'main' }]);
  });
