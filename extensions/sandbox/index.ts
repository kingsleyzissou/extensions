...
function execStream(
  ...
  command: string,
  {
    ...
    signal,
    timeout,
  }: { onData: (b: Buffer) => void; signal?: AbortSignal; timeout?: number },
    ...
    detectImageMimeType: async p => {
      ...
      try {
        const r = await execCapture(
          sbx,
          `file --mime-type -b ${shq(toRemote(p, sbx.hostCwd, sbx.mounts))}`,
        );
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerFlag('container-size', {
    description:
      'Sandbox size tier: xs, sm (default), md, lg, xlg, xxlg. Sets memory, swap, cpu, disk limits.',
    type: 'string',
    default: 'sm',
  });
  pi.registerFlag('sandbox-name', {
    description:
      'Re-usable sandbox name. If container exists, reattaches; otherwise creates new. Random name if omitted.',
    type: 'string',
  });
  ...
  pi.registerFlag('container-net', {
    description:
      'Allow outbound network from the sandbox (default: on; use --no-container-net to disable)',
    type: 'boolean',
    default: true,
  });
  ...
  pi.registerFlag('container-mount-skills', {
    description:
      'Mount agent skill directories read-only into the container at /skills (default: on)',
    type: 'boolean',
    default: true,
  });
  pi.registerFlag('container-mount-paths', {
    description:
      'Comma-separated list of additional host directories to mount read-only at /skills/<basename>',
    type: 'string',
  });
  pi.registerFlag('container-allow-paths', {
    description:
      'Comma-separated list of host path prefixes to allow for read access from outside the sandbox (e.g. ~/Downloads)',
    type: 'string',
  });
  ...
  pi.on('session_start', async (_event, ctx) => {
    ...
    try {
      ...
      const allowNetwork =
        (pi.getFlag('container-net') as boolean) || (pi.getFlag('browser') as boolean);
      ...
      let isReattached = false;
      {
        ...
        if (running) {
          ...
          const inspect = await spawnWithTimeout(
            runtime.bin,
            ['inspect', '--format', 'exists', sandboxName],
            5000,
          );
          if (inspect.code === 0 && !inspect.timedOut) {
            ...
            if (isReusable || persist) {
              ...
              if (started) {
                ...
                ctx.ui.notify(
                  `Removing broken container ${sandboxName}, creating fresh...`,
                  'warning',
                );
      ...
      const allowedExternalPrefixes = allowPathsRaw
        ? allowPathsRaw
            ...
            .map(p => p.trim())
            .filter(Boolean)
            .map(p => (p.startsWith('~') ? resolvePath(homedir(), p.slice(1)) : resolvePath(p)))
        : [];
      ...
      ctx.ui.setStatus(
        ...
        ctx.ui.theme.fg(
          'accent',
          `${statusPrefix}: ${actualName} (net=${allowNetwork ? 'on' : 'off'})${resStr}`,
        ),
      );
      ctx.ui.notify(
  ...
  async function cmdSandboxStatus(_subArgs: string, ctx: { ui: ExtensionUIContext }) {
    ...
    if (sbx.resources?.pidsLimit !== undefined)
      resParts.push(`pids-limit: ${sbx.resources.pidsLimit}`);
    const resStr = resParts.length ? `\nresources: ${resParts.join(', ')}` : '';
    const reusableStr = sbx.isReusable
      ? ` [re-usable${sbx.isReattached ? ', reattached' : ''}]`
      : '';
  ...
  async function cmdSandboxPathsList(ctx: { ui: ExtensionUIContext }) {
    ...
    if (records.length === 0) {
      ctx.ui.notify(
        'No persisted path approvals. External reads will prompt interactively.',
        'info',
      );
      return;
    }
    const lines = records.map(r => {
      const expiry =
        r.expiresAt === Infinity ? 'always' : `expires ${new Date(r.expiresAt).toISOString()}`;
  ...
  async function cmdSandboxPathsRevoke(target: string, ctx: { ui: ExtensionUIContext }) {
    ...
    const abs = target.startsWith('~')
      ? resolvePath(homedir(), target.slice(1))
      : resolvePath(target);
    if (store.revoke(abs)) {
  ...
  pi.registerCommand('sandbox', {
    description:
      'Sandbox management. Subcommands: status, doctor, allow, paths, install, update, config, pin, unpin',
